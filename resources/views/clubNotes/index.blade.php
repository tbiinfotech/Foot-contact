...
<div class="col sec_cont club-body">
   <div class="row">
      <div class="head_top">
         <h1> Club Notes </h1>
         <div class="dropdown">
         <a class="btn drk-btn" href="{{ route('club-notes-input') }}" role="button" >
                        Import 
                        </a>
            <a class="btn btn-secondary dropdown-toggle" href="{{ route('club-notes-export') }}" role="button">
               Export
            </a>
            <a class="btn btn-secondary dropdown-toggle" href="{{ route('club-notes-add') }}" role="button">
               + Add Club Note
            </a>
         </div>
      </div>
            ...
            @foreach($data as $key=>$item)
               ...
               <td scope="row">{{ ($item->currentpage()-1) * 10 + $key + 1 }}</td>
                  ...
                  <td>
                     ...
                     <a href="{{ route('club-notes-view', ['id' => $item->id]) }}" type="button" class="" data-id="{{$item->id}}" data-bs-target="#staticBackdrop">
                     ...
                     <ul class="dropdown-menu" aria-labelledby="dropdownMenuEdit">
                        <li><a class="dropdown-item" href="{{ route('club-notes-edit', ['id' => $item->id]) }}">Edit
                           </a>
                        </li>
                        <li><a id="delete_data" class="dropdown-item" href="{{ route('club-notes-delete', ['id' => $item->id]) }}">Delete</a></li>
                        @if($item->status==0)
                        <li><a class="dropdown-item" href="{{ route('club-notes-status', ['id' => $item->id]) }}">Inactive</a></li>
                  @else
                  <li><a class="dropdown-item" href="{{ route('club-notes-status', ['id' => $item->id]) }}">Active</a></li>
                  @endif
                     </ul>
                  </td>
               </tr>
